 ...
 * @param {string} [options.host='127.0.0.1'] - Remote host to connect to, if not given connects to 127.0.0.1.
...
const _closeBrowser = async () => {
    ...
    const waitForChromeToClose = Promise.all(
        new Promise((resolve) => {
            chromeProcess.once('close', () => {
                console.log('closed occurred');
                resolve();
            });
        }),
        new Promise((fulfill) => {
